...
def train_5fold(config, X_all, y_all, smiles_all, steric_all, verbose=True):
    ...
    for fold_idx, (train_idx, val_idx) in enumerate(kf.split(X_dev)):
        ...
        results, model, _ = train_single_model(
...
BASE_CONFIG = {
    ...
    'lambda_steric': 1.0,    # raised from 0.1 → 1.0 to enforce steric exclusion
    'lambda_bounds': 0.1,    # raised from 0.01 → 0.1 to enforce [0,100] bounds
    'use_huber': True,       # enable Huber loss for robustness to outliers
    'huber_delta': 5.0,
    'seed': 42
}
...
def run_evolutionary_search(max_generations=50, mutations_per_gen=3,
    ...
    try:
        results, model, scaler = train_single_model(
        ...
        # Fix 3: Save the scaler so inference reloads it exactly as trained
        # (avoids refitting on full dataset which shifts min/max bounds)
        import joblib
        ...
        for cand_idx in range(mutations_per_gen):
            ...
            try:
                ...
                if is_better:
                    ...
                    # Save updated scaler alongside the new best model
                    import joblib
                    scaler_path = os.path.join(base_dir, "checkpoint", "scaler.pkl")
                    joblib.dump(scaler, scaler_path)
